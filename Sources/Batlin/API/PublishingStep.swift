...
public extension PublishingStep {
    ...
    static func addTextbundleFiles(at path: Path = "Content", targetFolderPath: Path? = nil) -> Self {
        step(named: "Add Textbundle files from '\(path) folder") { context in 
            let folder = try context.folder(at: path)
            try await TextbundleFileHandler().addTextbundles(in: folder, to: &context, targetFolderPath: targetFolderPath)
        }
    }
